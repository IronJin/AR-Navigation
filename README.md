...
## 다국어 설정 방법
본 어플리케이션에서는 한국어, 영어, 중국어를 지원하며, 해당 사항을 설정하기 위해서는 환경설정을 구성해야 한다.
안드로이드에서는 언어설정을 구현함에 있어 버튼, 옵션 메뉴 보다는 환경설정 창을 이용해서 변경하는 것을 권장하고 있다.

다국어 설정은 기본적으로 MainActivity -> PreferenceActivity -> MySettingsFragment 순서대로 이루어진다.

 1. 다국어 설정을 위해 res/string에서 새 values 파일을 만든다. 이때 Available qualifiers를 Locale로 지정 한 후
...
<resources>
    ...
    <string name="user_location_permission_not_granted">위치권한실패 종료함</string>
...
<resources>
    ...
    <string name="user_location_permission_not_granted">Location Load Fail</string>
...
<resources>
    ...
    <string name="user_location_permission_not_granted">位置授权失败退出</string>
    <string name="new_location">新位置 : </string>
</resources>
```
3. 환경설정 화면 구성을 위해 res파일 안에 xml폴더를 만든 후 xml폴더 안에 새 xml파일을 만든다.
...
<PreferenceScreen
    ...
    <PreferenceCategory>
        <ListPreference
            ...
            android:summary="@string/language_setting_de"
    ...
    <PreferenceCategory>
        <Preference
            app:key="version"
            app:summary="Version 1.0 / made by 컴갤"
...
preferencescreen에서 사용되는 여러 태그들은 많은 자료들이 있음으로 필요한 태그를 이용하여 사용할 태그를 작성한다.
...
<FrameLayout
    ...
    <fragment
        ...
        android:name="com.example.mapbox.MySettingsFragment"
        />

</FrameLayout>
```
기본적인 프레임 아웃을 생성해준다. 이떄 fragment의 이름은 MySettingsFragment로 지정해 주어야 한다.

6. java에 MySettingsFragement, PreferencesActivity클래스를 생성한 후
PreferencesActivity에 AppCompatActivity를 상속 한다.

그 후 setContentView에서 사용할 레이아웃을 5번에 생성했던 파일명으로 지정해 준다.

7. MainActivity에서 이전에 menu를 활용하기 위해서
...
8. 이전에 만들었던 MySettingsFragment에서 PreferenceFragmentCompat을 상속한다.
이전에 많은 개발이 preference를 활용하였지만, 내가 사용하는 안드로이드 버전에서는
...
```
환경설정은 context객체가 없음으로 일반적으로 getActivity를 사용하는데 언어설정을 위해서라면,
앱을 재실행 해야한다. 그럴 경우 생명주기로 인해 getActivity에서 null을 반환하는 오류가 생기가 되는데
이를 방지하고자 onAttach를 활용하여 재실행한 activity를 받아와야한다.

해당 사항을 모두 완료하면, 아래와 같이 언어변경이 됨을 알 수 있다.

<img width="70%" src="https://user-images.githubusercontent.com/63465578/125032677-483df380-e0c9-11eb-97bb-608db42ebdad.gif
"/>
...
2. 목적지 위치의 위도, 경도 좌표를 데이터베이스를 통해 관리하면 더욱 편할 것 같음
